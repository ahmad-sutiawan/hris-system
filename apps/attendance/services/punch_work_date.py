...
def _cross_day_checkout_cutoff(assignment) -> time:
    """Latest local clock-out time still tied to the shift work_date (incl. grace)."""
    sched_out = assignment.scheduled_check_out
    grace = int(getattr(assignment.shift, "grace_period_minutes", 0) or 0)
    cutoff = datetime.combine(date.min, sched_out) + timedelta(minutes=grace)
    return cutoff.time()
...
def _in_cross_day_morning_tail(assignment, local_time: time) -> bool:
    ...
    if not assignment or not assignment.shift.cross_day:
        return False
    return local_time <= _cross_day_checkout_cutoff(assignment)
